...
Credential injection runs through a daemon-shipped `GIT_ASKPASS` helper fed by daemon-owned local IPC (never argv or env).
...
Endpoint policy starts formula-owned and is now also adjustable through the host-held remote controller.
The guest receives `GitRemote` only.
The host can keep `GitRemoteController` and `GitCredentialController` facets for policy updates, credential rotation, and revocation; `GitRemoteSet` remains optional follow-up work.
...
Phase 5 (see *Implementation Plan*) adds host-held controller accessors for revocation / policy updates and credential rotation.
The current implementation exposes `getGitRemoteController(remote)` and `getGitCredentialController(credential)` on the host surface.
Remote controller policy edits and revocation update the persisted `git-remote` formula so they survive daemon restart.
Controllers also expose a bounded in-memory `audit()` surface for creation, policy changes, revocation, operation outcomes, outbound ref updates, and rejected attempts; durable audit storage remains follow-up work.
Remote operations are fenced so a remote revoke, policy edit, or credential rotation/revocation that happens while native git is in flight prevents a stale success result from being reported.
Active native fetch/push subprocesses receive an `AbortSignal`; remote revocation and credential rotation/revocation abort the in-flight transport path, and `pull()` fences again before merge/rebase so a revoked remote cannot continue into local integration.
...
The current controller keeps this as a bounded in-memory log; a durable audit sink remains a follow-up for long-running operator workflows.
...
- a daemon-shipped `GIT_ASKPASS` helper binary, exec'd by `git` and fed the credential through daemon-owned local IPC (anonymous pipe, inherited fd, or per-operation Unix-domain socket; the secret is passed over that channel, never via argv or process env);
...
The **IPC-backed askpass** answers *how one native-git invocation receives the secret without disk, env, or argv exposure*: it is the in-process injection envelope, not a place to durably store anything.
...
The askpass-fed-by-daemon-IPC mechanism above is the only path that meets that bar for the basic (username/password) case.
...
Bearer-token support is therefore **conditional on the credential-injection spike** confirming an askpass / daemon-IPC path that keeps the bearer token out of argv, env, and any persisted location.
...
1. Whether the daemon-IPC-fed `GIT_ASKPASS` helper works on every target host's stock `git` (≥ 2.30; see [daemon-git-capability](daemon-git-capability.md) for the version pin), including under `git`'s recent `setup_credential_helpers` defaults.
2. Whether the askpass-fed-by-daemon-IPC path can carry a bearer token (treating the bearer as a password fed through askpass) while keeping the token out of argv, `/proc/*/environ`, and any persisted temp-file artifact a panicked git invocation might leave behind.
...
4. Whether the chosen IPC channel's handles are scoped to the helper: for fd-backed variants, `pipe2(O_CLOEXEC)` (Linux) and equivalent (macOS `pipe` + `fcntl(FD_CLOEXEC)`) must prevent credential fds from leaking into sibling subprocesses; for socket-backed variants, the per-operation socket path must be unguessable, removed after the invocation, and never contain the secret.
...
- [x] Add `GitRemote` and credential-capability types (`BearerCredential`, `BasicCredential`).
- [x] Add `git-remote` formula type bound to a local `Git`.
- [x] Add a host method to mint a `GitRemote` with policy baked in at construction (`provideGitRemote({...})`), including fetch-only, push-limited, and branch-limited validation.
- [x] The minimum viable agent flow (fetch + ff-only-pull + branch-limited push) is exercised end-to-end on this surface, with no controller in sight.
  Controllers come in Phase 5.

### Phase 2: HTTPS Credentialed Fetch

- [x] Support HTTPS bearer/basic credential injection through trusted backend code.
- [x] Implement `fetch()` with fixed endpoint and approved refspecs.
- [x] Keep packfile transfer on the HTTPS/native-git data plane rather than relaying git object bytes through CapTP.
...
- [x] Add controller audit entries for outbound ref updates (bounded in-memory; durable sink deferred).
- [x] Add local native data-plane tests for publishing `agent/*` branches; live HTTPS smart-HTTP coverage remains in the workflow test plan.

### Phase 5: Controllers and Revocation

- [x] Add `GitRemoteController` and `GitCredentialController` for post-construction policy updates, credential rotation, and revocation.
- [ ] Add `GitRemoteSet` if a collection capability is useful (host can also defer this).
- [x] Wire `revoke()` and credential changes against in-flight operations with result fencing and native subprocess abort.
- [x] The agent-facing surface from Phase 1 does not change; controllers add a parallel host-held authority for ops-team work.
