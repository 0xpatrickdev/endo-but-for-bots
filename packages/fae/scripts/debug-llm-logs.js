  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (head === '--state') {
      ...
      if (!Number.isFinite(parsed) || parsed < 0) {
        throw new Error(`--since must be a non-negative integer, got: ${value}`);
      }
...
const parseJsonBlock = (lines, startIndex, firstText) => {
  ...
  for (let i = startIndex + 1; i < lines.length && i < startIndex + 250; i += 1) {
  ...
  for (let i = 0; i < events.length; i += 1) {
    ...
    if (!next || next.kind !== 'toolResults') {
      findings.push({
        severity: 'medium',
        line: event.line,
        message: 'assistant tool_calls were not followed by logged tool results',
      });
      continue;
    }
    ...
    if (
      ...
      Array.isArray(
        /** @type {{ tool_calls?: unknown }} */ (normalizedAssistant).tool_calls,
      )
    ) {
...
const summarizeLog = log => {
  ...
  const bumpTool = (/** @type {string} */ name, /** @type {keyof {calls:0,ok:0,error:0}} */ key) => {
...
const printReport = (report, options) => {
  ...
  for (const log of report.logs) {
    ...
    for (const finding of log.toolFindings) {
      console.log(
        `[${finding.severity}] L${finding.line}: ${finding.message}`,
      );
    }
