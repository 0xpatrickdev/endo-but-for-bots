...
const readFileSpecifier = new URL('../tools/read-file.js', import.meta.url).href;
const timestampSpecifier = new URL('../tools/timestamp.js', import.meta.url).href;
...
export const runTrial = async ({ example, systemPrompt }) => {
  ...
  try {
    ...
    const providerId = /** @type {string} */ (await E(host).identify('default'));
    await E(host).provideGuest('fae-factory-handle', {
      introducedNames: harden({ '@agent': 'host-agent' }),
      agentName: 'profile-for-fae-factory-handle',
    });
    const factoryPowers = await E(host).lookup('profile-for-fae-factory-handle');
