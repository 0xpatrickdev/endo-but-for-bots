...
test('fixtures capture current prompt improvement over llm tip', async t => {
  ...
  const beforeCall =
    /** @type {any} */ (beforeRounds[0].response.message).tool_calls[0];
  const afterCall =
    /** @type {any} */ (afterRounds[0].response.message).tool_calls[0];
  ...
  const currentPrompt = await buildInboxMessageContent(
    {
      number: 1,
      type: 'package',
      strings: [
  ...
  t.true(currentPrompt.includes('Attached references'));
  t.true(
    currentPrompt.includes(
      'adoptTool(messageNumber=1, edgeName="math-tool", toolName="math-tool")',
    ),
  );
});
