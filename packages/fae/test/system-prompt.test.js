...
test('guest system prompt is assembled from named sections', t => {
  ...
  t.true(guestSystemPrompt.endsWith(guestSystemPromptSections.responseGuidelines));
});
...
test('guest system prompt teaches petname and tool naming boundary', t => {
  t.true(guestSystemPromptSections.namingConventions.includes('timestamp-tool'));
  t.true(guestSystemPromptSections.namingConventions.includes('timestampTool'));
  t.true(guestSystemPromptSections.adoption.includes('camelCase function name'));
});
