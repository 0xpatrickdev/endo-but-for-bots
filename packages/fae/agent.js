  ...
  const runAgenticLoop = async (
    ...
    while (continueLoop) {
      ...
      if (toolCalls.length !== 0) {
        const requiresAdoptionAttempt =
          unattemptedAdoptionEdges.size > 0;
        const {
          results: toolResults,
          attemptedAdoptionEdges,
        ...
        const finalNode = await tree.addNode(currentLeafId, [
          responseMessage,
        ]);
