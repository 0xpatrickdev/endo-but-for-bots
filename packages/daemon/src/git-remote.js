...
const normalizeDirections = value => {
  ...
  return harden(
    /** @type {GitDirection[]} */ ([...new Set(directions)]),
  );
};
...
const assertQualifiedRef = (ref, fieldName) => {
  if (!ref.startsWith('refs/')) {
    throw new Error(`${fieldName} must be fully qualified under refs/: ${q(ref)}`);
  }
  if (
    ref.includes('..') ||
    ref.includes('\\') ||
...
export const makeGitRemote = ({
  ...
  const persistState = async (nextPolicy, nextRevoked) => {
    ...
    if (onStateChange !== undefined) {
      await onStateChange(
        harden({ policy: nextPolicy, revoked: nextRevoked }),
      );
    }
  };
  ...
  const assertOperationFence = (operation, fence) => {
    if (operationEpoch !== fence.epoch) {
      ...
      throw new Error(`GitRemote ${q(name)} policy changed during ${operation}`);
    }
    if (requiresCredential && credentialRecord !== undefined) {
  ...
  const recordOperationSuccess = (type, result) => {
    const record = /** @type {{ updatedRefs?: unknown, fetch?: { updatedRefs?: unknown }, integration?: unknown, head?: unknown }} */ (
      result
    );
    const updatedRefs =
  ...
  const recordOperationFailure = (type, err) => {
    recordAudit({
      type,
      outcome: 'error',
      message: /** @type {{ message?: string }} */ (err)?.message || String(err),
    });
  };
  ...
  const pushRefspecsFromOptions = options => {
    ...
    return harden({ refspecs: harden([refspec]), setUpstream: !!opts.setUpstream });
  };
