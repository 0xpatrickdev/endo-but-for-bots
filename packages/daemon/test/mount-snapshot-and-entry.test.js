...
/**
 * Closes three open Phase 1 / Phase 2 checklist items from
 * `designs/daemon-mount-capabilities.md` that the Phase 5 dispatch
 * folds in:
 *
 *  - Phase 1 § Snapshot round-tripping: binary file streaming
 *  - Phase 1 § Snapshot round-tripping: symlink confinement behavior
 *  - Phase 2 § Descriptor provenance: read-only-entry
 *    write-authority-regain test
 *
 * All three are unit tests over `makeMount` directly so they do not
 * pay the daemon-fork cost.
 */
...
// --- Phase 2 § Descriptor provenance: read-only entry cannot regain write authority ---
